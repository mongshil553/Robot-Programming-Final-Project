...
class myActions(Node):
    ...
    async def action_hello(self, goal_handle):
        ...
        # res = self.servo_client.call(request)
        # print(res.message)
        #print(goal_handle.request.dummy_goal)
        goal_handle.succeed()
    ...
    async def action_kitchen(self, goal_handle):
        ...
        #     goal_handle.succeed()
        #     return Moving.Result(moving_result="Arrived at the kitchen")
        if result.status == 4:  # SUCCEEDED
            goal_handle.succeed()
            # return Moving.Result(moving_result="Arrived at the kitchen & pick object")
            return Moving.Result(moving_result="kitchen")
        else:
            goal_handle.abort()
            return Moving.Result(moving_result=f"Nav2 error: {result.status}")
